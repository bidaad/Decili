        ...
        private void ParsianComplete(string strAuthority, string strStatus)
        {
            ...
            if (CurTrans != null)
            {
                ...
                if (Status == 0)
                {
                    ...
                    else if (CurTrans.HCPayReasonCode == 8)//ارتقای عضویت
                    {
                        string strMembershipName = "";
                        int UserCode = (int)CurTrans.UserCode;
                        BOLUsers UsersBOL = new BOLUsers();
                        bool UpdateUserResult = UsersBOL.UpdateMembership(UserCode, CurTrans.MemTypeCode, CurTrans.MemLen);

                        DateTime NewExpDate;
                        switch ((int)CurTrans.MemLen)
                        {
                            case 1:
                        ...
                        switch ((int)CurTrans.MemTypeCode)
                        {
                            case 2:
                        ...
                        msgMessage.Text += " عضویت شما تا تاریخ " + dtm.GetPersianDate(NewExpDate) + " بصورت " + strMembershipName + " تمدید شد.";
                    }
